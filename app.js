...
window.onYouTubeIframeAPIReady=()=>{ytPlayer=new YT.Player('youtubePlayer',{videoId:'rqF-W9QBHUQ',playerVars:{autoplay:0,controls:0,disablekb:1,loop:1,playlist:'rqF-W9QBHUQ',playsinline:1,rel:0},events:{onReady:e=>{ytReady=true;document.querySelector('#musicStatus').textContent='背景音樂已就緒；若未自動播放，可展開播放器手動播放。';e.target.setVolume(Math.round(state.master*32));if(ytWanted&&state.playing)e.target.playVideo()},onError:e=>{console.warn('YouTube background music unavailable',e.data);document.querySelector('#musicStatus').textContent='YouTube 暫時無法播放；海洋聲景仍可使用。'}}})};
const youtubeScript=document.createElement('script');youtubeScript.src='https://www.youtube.com/iframe_api';youtubeScript.onerror=()=>{document.querySelector('#musicStatus').textContent='音樂連線失敗；海洋聲景仍可使用。'};document.head.append(youtubeScript);
...
function createAudio(){
  ...
  reverb.buffer=impulse;reverb.connect(wet).connect(master);master.connect(analyser).connect(compressor).connect(A.destination);
  ...
  const noiseBuffer=A.createBuffer(1,A.sampleRate*6,A.sampleRate),nd=noiseBuffer.getChannelData(0);let brown=0;for(let i=0;i<nd.length;i++){brown=(brown+.018*(Math.random()*2-1))/1.018;nd[i]=brown*3.5}
  ...
  const timer=setInterval(()=>{if(!state.playing||A.state!=='running')return;const now=A.currentTime;
    ...
    if(now>=nextBell&&state.jellies>.02){const chord=chords[(chordIndex-1+chords.length)%chords.length],hz=chord[Math.floor(Math.random()*chord.length)]*[2,3,4][Math.floor(Math.random()*3)];voice(bell,hz,now,2.4,'sine',.13);voice(bell,hz*2.01,now,1.3,'sine',.032);nextBell=now+rnd(.8,2.2)}
    ...
    if(now>=nextLifeTone){
      // A minor pentatonic: independent, density-controlled species voices.
      const scale=[0,3,5,7,10],hz=110*Math.pow(2,scale[lifeStep%5]/12);
      if(state.fish>.02)voice(bell,hz*4,now,.9,'sine',.07*state.fish);
      if(state.jellies>.02){voice(pad,hz*2,now+.12,3.8,'sine',.10*state.jellies);voice(bell,hz*3,now+.25,2.6,'sine',.035*state.jellies)}
      if(state.large>.12)voice(whales,hz*.5,now,4.2,'sine',.11*state.large);
      const density=(state.fish+state.jellies+state.large)/3;
      lifeStep++;nextLifeTone=now+2.8-density*1.8;
    }
    ...
    analyser.getByteFrequencyData(data);audioPulse=data.reduce((s,v)=>s+v,0)/(data.length*255);
  },90);
...
let yaw=.32,pitch=.38,slosh=0,sloshV=0,ripple=0,drag=null,pointerNX=0,pointerNY=0,pointerActive=0,impact=0,pointerTX=0,pointerTY=0,pointerInside=false;
const eventEnergy={whale:0,dolphin:0,turtle:0,jelly:0};
const innerR=.968,TAU=Math.PI*2;
function glassSphere(){const mobile=W<760,r=Math.min(W,H)*(mobile?.355:.405)*(0.82+state.zoom*.2);return{x:mobile?W*.5:W*.715,y:mobile?H*.39:H*.47,r}}
...
function ink(path,alpha=.85,fill=true){if(fill){ctx.fillStyle='rgba(2,12,23,.42)';ctx.fill(path);ctx.fillStyle='rgba('+drawColor+','+(alpha*.075*drawGlow)+')';ctx.fill(path)}ctx.shadowColor='rgba('+drawColor+','+(.38*drawGlow)+')';ctx.shadowBlur=6*drawGlow;ctx.strokeStyle=lineColor(alpha);ctx.stroke(path);ctx.shadowBlur=0}
...
function residents(s){const o=[],add=(kind,x,y,z,size,ph,ang=0,type='')=>{const follow={fish:.15,dolphin:.105,turtle:.06,jelly:.048,whale:.035,trash:0}[kind]||0,depthFollow=.62+.38*Math.max(0,Math.min(1,(z+1)/2));const ex=pointerNX*follow*pointerActive*depthFollow,ey=-pointerNY*follow*pointerActive*depthFollow;x+=ex*Math.cos(yaw)+ey*Math.sin(pitch)*Math.sin(yaw);z+=ex*Math.sin(yaw)-ey*Math.sin(pitch)*Math.cos(yaw);y+=ey*Math.cos(pitch);x+=Math.sin(t*2+ph)*.12*(eventEnergy[kind]||0);if(kind!=='trash'){const wobble=impact*(kind==='fish'?.07:kind==='jelly'?.055:.035);x+=Math.sin(t*7.5+ph*1.7)*wobble;y+=Math.cos(t*6.2+ph*1.3)*wobble*.7;ang+=Math.sin(t*8+ph)*impact*.12}const ceiling=Math.min(waterLevel()-.09,-.12),bottom=-innerR+size*1.3;y=Math.max(bottom,Math.min(ceiling,y+(waterLevel()+.15)*.55));const margin=size*1.35,d=Math.hypot(x,y,z),lim=innerR-margin;if(d>lim){const q=lim/d;x*=q;y*=q;z*=q}const top=Math.max(-.82,waterLevel()-.08-visual.wave*.10-Math.abs(slosh)*.15),half=Math.max(.035,(top+innerR)*.5),center=-innerR+half,fit=Math.min(1,half/(size*1.5)),safe=size*fit*1.25;size*=fit;y=Math.max(-innerR+safe,Math.min(top-safe,y));const ring=Math.sqrt(Math.max(0,(innerR-safe)**2-y*y)),rr=Math.hypot(x,z);if(rr>ring){x*=ring/rr;z*=ring/rr}o.push({kind,p:project(x,y,z,s),size,ph,ang,type})};
 ...
 o.sort((a,b)=>a.p.z-b.p.z);for(const x of o){drawColor=OCEAN_COLORS[x.kind]||OCEAN_COLORS.fish;drawDepth=x.p.z;drawGlow=x.kind==='trash'?.72:1.28;ctx.save();ctx.translate(x.p.x,x.p.y);ctx.rotate(x.ang);const sc=x.size*s.r;ctx.scale(sc,sc);ctx.lineWidth=Math.max(.009,1.05/sc);ctx.lineJoin='round';ctx.lineCap='round';if(x.kind==='whale')whaleShape(x.ph);else if(x.kind==='dolphin')dolphinShape(x.ph);else if(x.kind==='turtle')turtleShape(x.ph);else if(x.kind==='jelly')jellyShape(x.ph);else if(x.kind==='trash')trashShape(x.type,x.ph);else{ctx.rotate(Math.sin(x.ph)*.07);ink(forms.fish,.72);eye(.45,-.015,.04)}ctx.restore()}drawColor=OCEAN_COLORS.water;drawDepth=0;drawGlow=.58}
const seeds=Array.from({length:90},(_,i)=>({a:i*2.399963,r:Math.sqrt((i+.5)/90)*.88,p:i*1.719}));
function bg(){const g=ctx.createRadialGradient(W*.7,H*.45,10,W*.7,H*.45,Math.max(W,H)*.8);g.addColorStop(0,visual.purity>.45?(state.deep?'#020c20':'#042330'):'#202320');g.addColorStop(.55,'#03151d');g.addColorStop(1,'#01080d');ctx.fillStyle=g;ctx.fillRect(0,0,W,H)}
function draw(){requestAnimationFrame(draw);drawColor=OCEAN_COLORS.water;drawDepth=0;drawGlow=.58;const now=performance.now(),dt=Math.min((now-last)/1000,.04);last=now;const step=state.playing?dt:0;if(document.hidden)return;if(state.playing)t+=dt*(.22+state.speed*1.5);for(const kind in eventEnergy)eventEnergy[kind]*=Math.exp(-step*.35);for(const k of['depth','current','purity','jellies','fish','large','trash','life','wave','light','zoom'])visual[k]+=(state[k]-visual[k])*(1-Math.exp(-dt*7));ripple*=Math.exp(-step*.9);impact*=Math.exp(-step*2.05);pointerActive+=((pointerInside?1:0)-pointerActive)*(1-Math.exp(-step*3));pointerNX+=(pointerTX-pointerNX)*(1-Math.exp(-step*3));pointerNY+=(pointerTY-pointerNY)*(1-Math.exp(-step*3));sloshV+=(-slosh*7-sloshV*2)*step;slosh+=sloshV*step;bg();const s=glassSphere();ctx.save();ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,TAU);ctx.clip();
...
function updatePointer(e){const s=glassSphere();mouseX=e.clientX;mouseY=e.clientY;pointerTX=Math.max(-.82,Math.min(.82,(e.clientX-s.x)/s.r));pointerTY=Math.max(-.82,Math.min(.82,(e.clientY-s.y)/s.r));pointerInside=Math.hypot(e.clientX-s.x,e.clientY-s.y)<s.r}
canvas.addEventListener('pointerdown',e=>{updatePointer(e);if(!pointerInside)return;canvas.setPointerCapture(e.pointerId);drag={x:e.clientX,y:e.clientY,m:0};startAudio().catch(console.error)});
...
canvas.addEventListener('pointerleave',()=>{if(!drag)pointerInside=false});
draw();
...
addEventListener('keydown',e=>{if(e.target.matches('input,textarea,select,button')||e.repeat)return;const map={q:'whale',w:'dolphin',e:'jelly',r:'turtle',a:'pollution',s:'current',d:'deep',f:'clean',' ':'play'};if(map[e.key.toLowerCase()]){e.preventDefault();event(map[e.key.toLowerCase()])}});updateHealth();