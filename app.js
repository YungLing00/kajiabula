...
function draw(){requestAnimationFrame(draw);drawColor=OCEAN_COLORS.water;drawDepth=0;drawGlow=.58;const now=performance.now(),dt=Math.min((now-last)/1000,.04);last=now;if(state.playing)t+=dt*(.22+state.speed*1.5);for(const k of['depth','current','purity','jellies','fish','large','trash','life','wave','light','zoom'])visual[k]+=(state[k]-visual[k])*(1-Math.exp(-dt*7));ripple*=Math.exp(-dt*.9);sloshV+=(-slosh*7-sloshV*2)*dt;slosh+=sloshV*dt;bg();const s=glassSphere();ctx.save();ctx.beginPath();ctx.arc(s.x,s.y,s.r,0,TAU);ctx.clip();
